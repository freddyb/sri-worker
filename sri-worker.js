var cacheName = 'sri-worker-0.0.1';
...
const INTEGRITY_LEVEL = 0; // >9000 means require-sri-for.
...
self.addEventListener('fetch', function(event) {
  ...
  if ((request.method == 'GET') &&
    ...
    console.info('[ServiceWorker] Adding integrity metadata for fetch', request.url, '('+sriHash+')');
    ...
    /* though if you want something like  that makes security guarantees about the
       integrity of *all* your scripts, then it should happen in the platform,
        not in JavaScript
    */
    console.error('[ServiceWorker] Can not fetch', request.url+".", "No integrity metadata.");
    event.respondWith(Response.error())
  } else {
    // else, fetch as is.
    console.warn('[ServiceWorker] Can not add integrity metadata for fetch', request.url);
